...
const sendAllStudentsJoined = (req, res) => {
  db(
    `SELECT students.first_name, students.last_name, students.group_id, behaviors.student_id, behaviors.has_goal_one, behaviors.has_goal_two, behaviors.has_goal_three, behaviors.score FROM students INNER JOIN behaviors ON students.id = behaviors.student_id;`
  )
    .then((results) => {
      res.send(results.data);
    })
...
router.get("/behaviors", (req, res) => {
  db(`SELECT * FROM behaviors`).then((results) => {
    res.send(results.data);
  });
});
...
router.post("/students", (req, res) => {
  ...
  db(
    `INSERT INTO students (first_name, last_name) VALUES ("${req.body.first_name}", "${req.body.last_name}");`
  )
    .then(() => {
      db(
        `SELECT id FROM students WHERE last_name="${req.body.last_name}" AND first_name="${req.body.first_name}"`
      ).then((results) => {
        let score =
          req.body.has_goal_one +
          req.body.has_goal_two +
          req.body.has_goal_three;
        db(
          `INSERT INTO behaviors (has_goal_one, has_goal_two, has_goal_three, student_id, score) SELECT ${req.body.has_goal_one}, ${req.body.has_goal_two}, ${req.body.has_goal_three}, ${results.data[0].id}, ${score} FROM students WHERE last_name="${req.body.last_name}" AND first_name="${req.body.first_name}";`
        ).then(() => {
          db(
            `SELECT students.first_name, students.last_name, students.group_id, behaviors.student_id, behaviors.has_goal_one, behaviors.has_goal_two, behaviors.has_goal_three, behaviors.score FROM students INNER JOIN behaviors ON students.id = behaviors.student_id;`
          ).then((results) => {
...
router.post("/students/file", fileUpload, (req, res) => {
  ...
  fs.createReadStream(path.join(__dirname, "../files/" + req.file.filename)) // csv-parser
    .pipe(csv())
    ...
    .on("end", () => {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        db(
          ...
          .then(() => {
            db(
              `SELECT id FROM students WHERE last_name="${results[i].last_name}" AND first_name="${results[i].first_name}"`
            ).then((result) => {
              db(
                `INSERT INTO behaviors (has_goal_one, has_goal_two, has_goal_three, student_id, score) SELECT ${results[i].has_goal_one}, ${results[i].has_goal_two}, ${results[i].has_goal_three}, ${result.data[0].id}, ${score} FROM students WHERE last_name="${results[i].last_name}" AND first_name="${results[i].first_name}";`
              ).then(() => {});
            });
          })
          .catch((error) => {
...
router.delete("/students/:id", (req, res) => {
  db(`DELETE from behaviors WHERE student_id = ${req.params.id};`)
    .then(() => {
      db(`DELETE FROM students WHERE id = ${req.params.id}`).then(() => {
        sendAllStudentsJoined(req, res);
      });
    })
    .catch((error) => {
